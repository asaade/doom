...
;;; Commentary:
;; Aesthetic emphasizing readability, clean UI,
...
(setq-default cursor-in-non-selected-windows nil ; Hide the cursor in inactive windows
              cursor-type '(bar . 2)            ; Underline-shaped cursor
...
(setq resolution-factor (* 1.1 (/ (display-pixel-height) 1080.0)))

(setq doom-font (font-spec :family user-font :weight 'normal :size (round (* 16 resolution-factor)))
      doom-variable-pitch-font (font-spec :family variable-font :weight 'normal :size (round (* 16 resolution-factor)))
      doom-big-font (font-spec :family user-font :weight 'normal :size (round (* 28 resolution-factor)))
      doom-symbol-font (font-spec :family symbols-font :size (round (* 18 resolution-factor))))
...
(defcustom variable-pitch-serif-font (font-spec :family "Merriweather" :size (round (* 18 resolution-factor)))
...
(defun my/apply-modern-theme (appearance)
  ...
  (pcase appearance
    ...
    ('dark (load-theme 'alabaster-themes-dark :no-confirm) t)))
...
(custom-set-faces!
  '(org-document-title :height 1.5 :weight light :slant normal)
  '(org-level-1 :height 1.3 :weight light)
  '(org-level-2 :height 1.2 :weight light)
